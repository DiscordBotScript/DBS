export default {
    async execute(utils) {
        ...
        let [name, value, userId = utils.source[utils.source.author ? 'author' : 'user'].id] = utils.inside.split(';')
        if(!name || name  === 'undefined') throw new Error('Missing Var Name')
        if(!value || value === 'undefined') throw new Error('Missing Var Value')
            name = `${userId}.${name}`
          let db;
            try {
         db = JSON.parse(await utils.fs.readFileSync(`${utils.dbFile}/Users.json`, 'utf8'))
        } catch {
            fs.writeFileSync(`${utils.dbFile}/Users.json`, JSON.stringify([]), () => { })
            db = JSON.parse(await utils.fs.readFileSync(`${utils.dbFile}/Users.json`, 'utf8'))
        }

            let result = await setNestedProperty(db, name, value)
            await utils.fs.writeFileSync(`${utils.dbFile}/Users.json`, JSON.stringify(result,null,2))
