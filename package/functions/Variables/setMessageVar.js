export default {
    async execute(utils) {
        ...
        let [name, value, mesId = utils.source.targetId ?? utils.source?.message?.id ?? utils.source.id] = utils.inside.split(';')
        if(!name || name  === 'undefined') throw new Error('Missing Var Name')
        if(!value || value === 'undefined') throw new Error('Missing Var Value')
            name = `${mesId}.${name}`
          let db;
            try {
         db = JSON.parse(await utils.fs.readFileSync(`${utils.dbFile}/Messages.json`, 'utf8'))
        } catch {
            fs.writeFileSync(`${utils.dbFile}/Messages.json`, JSON.stringify([]), () => { })
            db = JSON.parse(await utils.fs.readFileSync(`${utils.dbFile}/Messages.json`, 'utf8'))
        }

            let result = await setNestedProperty(db, name, value)
            await utils.fs.writeFileSync(`${utils.dbFile}/Messages.json`, JSON.stringify(result,null,2))
